...
public class DomainStateHandler implements StateHandler {
    ...
    public void handle(Message message, UserStateService stateService) {
        ...
        List<String> domains = Arrays.stream(message.getText().split(","))
                .map(String::trim)
                .filter(s -> !s.isEmpty())
                .collect(Collectors.toList());

        sessionData.setDomains(domains);
        stateService.setState(chatId, BotState.AWAITING_TOPIC_CONFIRMATION);
        bot.sendTopicConfirmationRequest(chatId);
    }
}
